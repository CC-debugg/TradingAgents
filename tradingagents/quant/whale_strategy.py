...
def latest_whale_signal(
    ...
    """Today's whale conviction signal for live dashboard / CLOB."""
    cfg = cfg or WhaleStrategyConfig()
    if flow.empty or prob.empty:
        return {"signal": 0.0, "flow_net_usd": 0.0, "n_trades": 0, "mode": "v2"}
    sig = whale_flow_signal_v2(flow, prob, cfg)
    last = sig.dropna().iloc[-1] if len(sig.dropna()) else 0.0
    tail = flow.iloc[-cfg.flow_window :]
    return {
        "signal": float(last),
        "flow_net_usd": float(tail["flow_net_usd"].sum()),
        "n_trades": int(tail["n_trades"].sum()) if "n_trades" in tail else 0,
